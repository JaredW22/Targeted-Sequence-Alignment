    ...
    for name, seq in array:
        ...
        for i in range(len(query_seq)):
            for j in range(i + min_overlap_length, len(query_seq) + 1):
                ...
                if substring in seq or rev_comp_substring in seq:
                    start_index = seq.find(substring) if substring in seq else seq.find(rev_comp_substring)
                    end_index = start_index + len(substring)
                    length = j - i
                    temp_overlaps.append((name, query_name, seq, i, j, start_index, end_index, length))
                    if query_seq[:min_overlap_length] in seq or query_seq[-min_overlap_length:] in seq:
