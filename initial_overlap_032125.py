    ...
    for name, seq in array:
        ...
        for i in range(len(query_seq)):
            for j in range(i + min_overlap_length, len(query_seq) + 1):
                ...
                rev_comp_substring = reverse_complement(substring)
                if substring in seq:
                    start_index = seq.find(substring)
                    end_index = start_index + len(substring)
                    length = j - i
                    temp_overlaps.append((name, query_name, seq, i, j, start_index, end_index, length))
                    if query_seq[:min_overlap_length] in seq or query_seq[-min_overlap_length:] in seq: #this is the chunk that is not working, 
                        combined_sequence = "Test_fwd" #query_seq[:i] + seq[start_index:end_index] + query_seq[j:]
                ...
                if rev_comp_substring in seq:
                    ...
                    temp_overlaps.append((name, query_name, seq, i, j, start_index, end_index, length))
                    if query_seq[:min_overlap_length] in seq or query_seq[-min_overlap_length:] in seq: #this is the chunk that is not working
                        combined_sequence = "Test_bkwd" #query_seq[:i] + seq[start_index:end_index] + query_seq[j:]
                        temp_contigs.append((name, query_name, combined_sequence, i, j, start_index, end_index, length))
        if temp_overlaps:
